use xilem_html::{
    cached, elements::*, get_element_by_id, interfaces::*, s, t, App, View, ViewMarker,
};
...
fn control_buttons() -> impl View<AppState> + ViewMarker {
    s(|| {
        ...
        div(div((
            div(h1(
                "xilem_html diff+cache+template+strong-typed (non-keyed)",
            ))
            .attr("class", "col-md-6"),
            div(div((
...
fn row(r: &Row, selected: Option<usize>) -> impl View<AppState> + ViewMarker {
    cached(
        (r.id, r.label.clone(), selected),
        |(prev_id, prev_label, prev_selected), (id, label, selected)| {
            let label_or_id_changed = prev_id != id || prev_label != label;
            let selected_changed = (*prev_selected == Some(*id)) != (*selected == Some(*id));

            label_or_id_changed || selected_changed
        },
        |(id, label, selected)| {
            let id = *id;
