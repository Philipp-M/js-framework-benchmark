use xilem_html::{
    elements::*, get_element_by_id, interfaces::*, memoize, s, App, View, ViewMarker,
};
...
impl AppState {
    ...
    fn update(&mut self) {
        ...
        for i in (0..(self.rows.len() / step)).map(|x| x * step) {
            let mut s = self.rows[i].label.clone().into_owned();
            s.push_str(" !!!");
            self.rows[i].label = CowStr::from(s);
        }
    }
    fn clear(&mut self) {
        self.rows.clear();
...
fn control_buttons() -> impl View<AppState> + ViewMarker {
    s(|| {
        ...
        div(div((
            div(h1("xilem_html strong-typed cow (non-keyed)")).attr("class", "col-md-6"),
            div(div((
