use xilem_html::{elements::*, get_element_by_id, t, App, View, ViewExt, ViewMarker, ViewSequence};
...
fn control_buttons() -> impl View<AppState> + ViewMarker {
    ...
    div(div((
        div(h1("xilem_html template (non-keyed)")).attr("class", "col-md-6"),
        div(div((
...
fn row(r: &Row, selected: Option<usize>) -> impl ViewSequence<AppState> {
    let id = r.id;
    ...
    .attr("class", (selected == Some(id)).then_some("danger")))
}
